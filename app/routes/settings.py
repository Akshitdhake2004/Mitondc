...
@settings_bp.route('/shipping/edit/<int:id>', methods=['POST'])
@login_required
def shipping_edit(id):
    """Edit a shipping method."""
    shipping_method = ShippingMethod.query.get_or_404(id)

    if request.method == 'POST':
        shipping_method.name = request.form.get('name')
        shipping_method.description = request.form.get('description')
        shipping_method.price = request.form.get('price', type=float)
        shipping_method.estimated_days = request.form.get('estimated_days')
        shipping_method.is_active = 'is_active' in request.form
